...
{
    public class InfluxcoinNetwork : BitcoinNetwork
    {
        protected InfluxcoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "Influxcoin"; }
        }

        public new static InfluxcoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<InfluxcoinNetwork> FNetworks;

        public new static InfluxcoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<InfluxcoinNetwork>();

                FNetworks.Add(new InfluxcoinNetwork(Network.MainNet));
                FNetworks.Add(new InfluxcoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (102) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (28) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (230) });
