...
{
    public class DashNetwork : BitcoinNetwork
    {
        protected DashNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static DashNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<DashNetwork> FNetworks;

        public new static DashNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<DashNetwork>();

                FNetworks.Add(new DashNetwork(Network.MainNet));
                FNetworks.Add(new DashNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (76) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (16) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (204) });
        ...
        {
            InitMainNet();
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (140) });
