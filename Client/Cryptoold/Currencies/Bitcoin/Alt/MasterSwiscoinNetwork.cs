...
{
    public class MasterSwiscoinNetwork : BitcoinNetwork
    {
        protected MasterSwiscoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "MasterSwiscoin"; }
        }

        public new static MasterSwiscoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<MasterSwiscoinNetwork> FNetworks;

        public new static MasterSwiscoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<MasterSwiscoinNetwork>();

                FNetworks.Add(new MasterSwiscoinNetwork(Network.MainNet));
                FNetworks.Add(new MasterSwiscoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (125) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (20) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (253) });
