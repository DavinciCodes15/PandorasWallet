...
{
    public class ReddCoinNetwork : BitcoinNetwork
    {
        protected ReddCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static ReddCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<ReddCoinNetwork> FNetworks;

        public new static ReddCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<ReddCoinNetwork>();

                FNetworks.Add(new ReddCoinNetwork(Network.MainNet));
                FNetworks.Add(new ReddCoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (61) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (5) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (189) });
