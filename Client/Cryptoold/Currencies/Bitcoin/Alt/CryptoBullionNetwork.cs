...
{
    public class CryptoBullionNetwork : BitcoinNetwork
    {
        protected CryptoBullionNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "CryptoBullion"; }
        }

        public new static CryptoBullionNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<CryptoBullionNetwork> FNetworks;

        public new static CryptoBullionNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<CryptoBullionNetwork>();

                FNetworks.Add(new CryptoBullionNetwork(Network.MainNet));
                FNetworks.Add(new CryptoBullionNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (11) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (8) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (139) });
