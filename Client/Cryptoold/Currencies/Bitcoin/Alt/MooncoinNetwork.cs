...
{
    public class MooncoinNetwork : BitcoinNetwork
    {
        protected MooncoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static MooncoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<MooncoinNetwork> FNetworks;

        public new static MooncoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<MooncoinNetwork>();

                FNetworks.Add(new MooncoinNetwork(Network.MainNet));
                FNetworks.Add(new MooncoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (3) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (22) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (131) });
