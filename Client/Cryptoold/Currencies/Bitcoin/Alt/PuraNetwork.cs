...
{
    public class PuraNetwork : BitcoinNetwork
    {
        protected PuraNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static PuraNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<PuraNetwork> FNetworks;

        public new static PuraNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<PuraNetwork>();

                FNetworks.Add(new PuraNetwork(Network.MainNet));
                FNetworks.Add(new PuraNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (150) });
        ...
        {
            InitMainNet();
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (204) });
