...
{
    public class IOCoinNetwork : BitcoinNetwork
    {
        protected IOCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static IOCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<IOCoinNetwork> FNetworks;

        public new static IOCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<IOCoinNetwork>();

                FNetworks.Add(new IOCoinNetwork(Network.MainNet));
                FNetworks.Add(new IOCoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (103) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (85) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (231) });
