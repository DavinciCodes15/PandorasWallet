...
{
    public class BitBeanNetwork : BitcoinNetwork
    {
        protected BitBeanNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static BitBeanNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<BitBeanNetwork> FNetworks;

        public new static BitBeanNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<BitBeanNetwork>();

                FNetworks.Add(new BitBeanNetwork(Network.MainNet));
                FNetworks.Add(new BitBeanNetwork(Network.TestNet));
            }
