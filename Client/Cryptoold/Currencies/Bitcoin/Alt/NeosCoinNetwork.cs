...
{
    public class NeosCoinCoinNetwork : BitcoinNetwork
    {
        protected NeosCoinCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "NeosCoinCoin"; }
        }

        public new static NeosCoinCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<NeosCoinCoinNetwork> FNetworks;

        public new static NeosCoinCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<NeosCoinCoinNetwork>();

                FNetworks.Add(new NeosCoinCoinNetwork(Network.MainNet));
                FNetworks.Add(new NeosCoinCoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (53) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (5) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (177) });
