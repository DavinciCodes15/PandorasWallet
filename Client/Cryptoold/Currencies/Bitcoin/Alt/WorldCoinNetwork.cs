...
{
    public class WorldCoinNetwork : BitcoinNetwork
    {
        protected WorldCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "WorldCoin"; }
        }

        public new static WorldCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<WorldCoinNetwork> FNetworks;

        public new static WorldCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<WorldCoinNetwork>();

                FNetworks.Add(new WorldCoinNetwork(Network.MainNet));
                FNetworks.Add(new WorldCoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (73) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (5) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (201) });
