...
{
    public class ECoinNetwork : BitcoinNetwork
    {
        protected ECoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static ECoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<ECoinNetwork> FNetworks;

        public new static ECoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<ECoinNetwork>();

                FNetworks.Add(new ECoinNetwork(Network.MainNet));
                FNetworks.Add(new ECoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (92) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (20) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (220) });
