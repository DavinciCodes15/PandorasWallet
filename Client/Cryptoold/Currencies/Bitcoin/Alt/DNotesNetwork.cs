...
{
    public class DNotesNetwork : BitcoinNetwork
    {
        protected DNotesNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static DNotesNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<DNotesNetwork> FNetworks;

        public new static DNotesNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<DNotesNetwork>();

                FNetworks.Add(new DNotesNetwork(Network.MainNet));
                FNetworks.Add(new DNotesNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (31) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (5) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (159) });
