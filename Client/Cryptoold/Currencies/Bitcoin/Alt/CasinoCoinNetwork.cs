...
{
    public class CasinoCoinNetwork : BitcoinNetwork
    {
        protected CasinoCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "CasinoCoin"; }
        }

        public new static CasinoCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<CasinoCoinNetwork> FNetworks;

        public new static CasinoCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<CasinoCoinNetwork>();

                FNetworks.Add(new CasinoCoinNetwork(Network.MainNet));
                FNetworks.Add(new CasinoCoinNetwork(Network.TestNet));
            }
        ...
        {
            InitMainNet();
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (87) });
