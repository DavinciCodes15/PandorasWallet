...
{
    public class PrimeCoinNetwork : BitcoinNetwork
    {
        protected PrimeCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "PrimeCoin"; }
        }

        public new static PrimeCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<PrimeCoinNetwork> FNetworks;

        public new static PrimeCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<PrimeCoinNetwork>();

                FNetworks.Add(new PrimeCoinNetwork(Network.MainNet));
                FNetworks.Add(new PrimeCoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (83) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (151) });
