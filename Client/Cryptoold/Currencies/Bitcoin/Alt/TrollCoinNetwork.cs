...
{
    public class TrollCoinNetwork : BitcoinNetwork
    {
        protected TrollCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "TrollCoin"; }
        }

        public new static TrollCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<TrollCoinNetwork> FNetworks;

        public new static TrollCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<TrollCoinNetwork>();

                FNetworks.Add(new TrollCoinNetwork(Network.MainNet));
                FNetworks.Add(new TrollCoinNetwork(Network.TestNet));
            }
