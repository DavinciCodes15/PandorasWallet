...
{
    public class VertcoinNetwork : BitcoinNetwork
    {
        protected VertcoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static VertcoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<VertcoinNetwork> FNetworks;

        public new static VertcoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<VertcoinNetwork>();

                FNetworks.Add(new VertcoinNetwork(Network.MainNet));
                FNetworks.Add(new VertcoinNetwork(Network.TestNet));
            }
        ...
        {
            InitMainNet();
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (74) });
