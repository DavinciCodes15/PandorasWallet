...
{
    public class FaircoinNetwork : BitcoinNetwork
    {
        protected FaircoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static FaircoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<FaircoinNetwork> FNetworks;

        public new static FaircoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<FaircoinNetwork>();

                FNetworks.Add(new FaircoinNetwork(Network.MainNet));
                FNetworks.Add(new FaircoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (95) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (36) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (223) });
