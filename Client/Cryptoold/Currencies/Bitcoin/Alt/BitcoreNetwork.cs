...
{
    public class BitcoreNetwork : BitcoinNetwork
    {
        protected BitcoreNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static BitcoreNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<BitcoreNetwork> FNetworks;

        public new static BitcoreNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<BitcoreNetwork>();

                FNetworks.Add(new BitcoreNetwork(Network.MainNet));
                FNetworks.Add(new BitcoreNetwork(Network.TestNet));
            }
