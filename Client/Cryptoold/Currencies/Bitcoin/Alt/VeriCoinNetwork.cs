...
{
    public class VeriCoinNetwork : BitcoinNetwork
    {
        protected VeriCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static VeriCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<VeriCoinNetwork> FNetworks;

        public new static VeriCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<VeriCoinNetwork>();

                FNetworks.Add(new VeriCoinNetwork(Network.MainNet));
                FNetworks.Add(new VeriCoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (70) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (132) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (198) });
