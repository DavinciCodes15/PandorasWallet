...
{
    public class HappyCoinNetwork : BitcoinNetwork
    {
        protected HappyCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "HappyCoin"; }
        }

        public new static HappyCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<HappyCoinNetwork> FNetworks;

        public new static HappyCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<HappyCoinNetwork>();

                FNetworks.Add(new HappyCoinNetwork(Network.MainNet));
                FNetworks.Add(new HappyCoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (41) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (20) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (169) });
