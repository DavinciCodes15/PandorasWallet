...
{
    public class AllionNetwork : BitcoinNetwork
    {
        protected AllionNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static AllionNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<AllionNetwork> FNetworks;

        public new static AllionNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<AllionNetwork>();

                FNetworks.Add(new AllionNetwork(Network.MainNet));
                FNetworks.Add(new AllionNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (26) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (8) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (154) });
