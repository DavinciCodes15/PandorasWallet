...
{
    public class YashCoinNetwork : BitcoinNetwork
    {
        protected YashCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static YashCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<YashCoinNetwork> FNetworks;

        public new static YashCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<YashCoinNetwork>();

                FNetworks.Add(new YashCoinNetwork(Network.MainNet));
                FNetworks.Add(new YashCoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (78) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (20) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (206) });
