...
{
    public class ZCoinNetwork : BitcoinNetwork
    {
        protected ZCoinNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        public new static ZCoinNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<ZCoinNetwork> FNetworks;

        public new static ZCoinNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<ZCoinNetwork>();

                FNetworks.Add(new ZCoinNetwork(Network.MainNet));
                FNetworks.Add(new ZCoinNetwork(Network.TestNet));
            }
        ...
        protected override void InitMainNet()
        {
            ...
            SetBase58Prefixes(Base58Type.PUBKEY_ADDRESS, new byte[] { (82) });
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (7) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (210) });
        ...
        protected override void InitTestNet()
        {
            ...
            SetBase58Prefixes(Base58Type.SCRIPT_ADDRESS, new byte[] { (178) });
            SetBase58Prefixes(Base58Type.SECRET_KEY, new byte[] { (185) });
