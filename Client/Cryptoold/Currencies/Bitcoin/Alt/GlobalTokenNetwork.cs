...
{
    public class GlobalTokenNetwork : BitcoinNetwork
    {
        protected GlobalTokenNetwork(int aNet)
            : base(aNet)
        {
        }
        ...
        {
            get { return "GlobalToken"; }
        }

        public new static GlobalTokenNetwork GetMainNet()
        {
            return GetNetworks()[MainNet];
        }

        private static List<GlobalTokenNetwork> FNetworks;

        public new static GlobalTokenNetwork[] GetNetworks()
        {
            if (FNetworks == null)
            {

                FNetworks = new List<GlobalTokenNetwork>();

                FNetworks.Add(new GlobalTokenNetwork(Network.MainNet));
                FNetworks.Add(new GlobalTokenNetwork(Network.TestNet));
            }
