...
namespace Pandora.Client.Crypto.Currencies
{
	...
	public class PubKey : ICoinSerializable, IDestination
	{
		...
		/// <param name="unsafe">If false, make public copy of bytes and does perform only a costly check for PubKey format. If true, the bytes array is used as is and only PubKey.Check is used for validating the format. </param>	 
		...
		public CoinPubKeyAddress GetAddress(Network network)
		{
			return network.CreateCoinAddress(this.Hash);
		}
		...
		public CoinScriptAddress GetScriptAddress(Network network)
		{
			...
			return new CoinScriptAddress(redeem.Hash, network);
		}
		...
		/// <summary>
		/// Verify message signed using signmessage from Coin
		/// </summary>
		...
		/// <summary>
		/// Decode signature from Coine verify/signing rpc methods
		/// </summary>
		...
		public PubKey Derivate(byte[] cc, uint nChild, out byte[] ccChild)
		{
			...
			q = q.Normalize();
            var p = new Pandora.Client.Crypto.Currencies.BouncyCastle.Math.EC.FpPoint(ECKey.CURVE.Curve, q.XCoord, q.YCoord, true);
        ...
        //public CoinStealthAddress CreateStealthAddress(PubKey scanKey, Network network)
        //{
        //    return new CoinStealthAddress(scanKey, new PubKey[] { this }, 1, null, network);
        //}

		public string ToString(Network network)
		{
			return new CoinPubKeyAddress(this.Hash, network).ToString();
		}
		...
		public CoinWitPubKeyAddress GetSegwitAddress(Network network)
		{
			return new CoinWitPubKeyAddress(WitHash, network);
		}
