...
namespace Pandora.Client.Crypto.Currencies
{
    public class TxIn : ICoinSerializable
    {
        ...
        {
            PubKey lItem = PayToPubkeyTemplate.Instance.ExtractScriptPubKeyParameters(ScriptSig);
        ...
        public static TxIn CreateCoinbase(int height)
        {
            TxIn txin = new TxIn
            {
                ScriptSig = new Script(Op.GetPushOp(height)) + OpcodeType.OP_0
            };
            return txin;
        }
    }
    ...
    public class IndexedTxIn
    {
        ...
        {
            return VerifyScript(coin, scriptVerify, out ScriptError error);
        }
        ...
        {
            uint256 hash = GetSignatureHash(coin, sigHash);
        ...
        {
            return Script.SignatureHash(Transaction.Network, coin.GetScriptCode(), Transaction, (int)Index, sigHash, coin.TxOut.Value, coin.GetHashVersion());
        }
    }
    ...
    public class WitScript
    {
        ...
        private void ReadCore(CoinStream stream)
        {
            List<byte[]> pushes = new List<byte[]>();
            uint pushCount = 0;
    ...
    public class Transaction : ICoinSerializable
    {
        ...
        public Transaction(Network aNetwork) : this()
        {
            Network = aNetwork;
        }

        public Transaction(string hex, Network aNetwork)
            : this(aNetwork)
        {
            if (aNetwork.ChainParams is IServerChainParams)
            {
                this.FromBytes(Encoders.Hex.DecodeData(hex), (uint)(aNetwork.ChainParams as IServerChainParams).MaxP2PVersion);
            }
            else
            {
            ...
            : this(aNetwork)
        {
            if (aNetwork.ChainParams is IServerChainParams)
            {
                this.FromBytes(bytes, (uint)(aNetwork.ChainParams as IServerChainParams).MaxP2PVersion);
            }
            else
            {
        ...
        public Money TotalOut => Outputs.Sum(v => v.Value);
        ...
        public void Sign(CCKey key, bool assumeP2SH)
        {
            ...
            for (int i = 0; i < Inputs.Count; i++)
            {
                ...
                if (assumeP2SH)
                {
                    PayToScriptHashSigParameters p2shSig = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(txin.ScriptSig);
                    if (p2shSig == null)
                    {
        ...
        public FeeRate GetFeeRate(ICoin[] spentCoins)
        {
            ...
            return new FeeRate(fee, this.GetSerializedSize(Version));
        }
        ...
        public Transaction WithOptions(TransactionOptions options)
        {
            ...
            Transaction instance = new Transaction(Network);
            MemoryStream ms = new MemoryStream();
