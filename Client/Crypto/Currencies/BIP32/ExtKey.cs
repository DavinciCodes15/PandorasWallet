...
namespace Pandora.Client.Crypto.Currencies.BIP32
{
	...
	/// </summary>
	public class ExtKey : ICoinSerializable, IDestination, ISecret, IEquatable<ExtKey>, IHDKey
	{
		/// <summary>
		...
		/// </summary>
		public ExtKey(BitcoinExtPubKey extPubKey, CoinSecret key)
		...
		/// </summary>
		public ExtKey(CCKey key, byte[] chainCode, byte depth, HDFingerprint fingerprint, uint child)
		{
			if (key == null)
		...
		public ExtKey Derive(RootedKeyPath rootedKeyPath)
		{
			...
			if (rootedKeyPath.MasterFingerprint != GetHDFingerPrint())
        ...
        private HDFingerprint GetHDFingerPrint()
        {
			return new HDFingerprint(GetPublicKey());
        }
		...
		public bool IsChildOf(ExtKey parentKey)
		{
			if (Depth != parentKey.Depth + 1)
				return false;
			return new HDFingerprint(parentKey.PrivateKey.PubKey) == ParentFingerprint;
		}
		...
		public ExtKey Derive(uint index)
		{
			var result = new ExtKey
			{
				...
				parentFingerprint = new HDFingerprint(this.key.PubKey),
				nChild = index
			};
