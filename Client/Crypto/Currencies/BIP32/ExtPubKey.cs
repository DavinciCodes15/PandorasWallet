...
namespace Pandora.Client.Crypto.Currencies.BIP32
{
	...
	/// A public HD key
	/// </summary>
	public class ExtPubKey : ICoinSerializable, IDestination, IHDKey
	{
		...
		public ExtPubKey(byte[] bytes)
		{
			...
			this.ReadWrite(new CoinStream(bytes));
		}
		...
		public ExtPubKey(byte[] bytes, int offset, int length)
		{
			if (bytes == null)
				throw new ArgumentNullException(nameof(bytes));
			this.ReadWrite(new CoinStream(new MemoryStream(bytes, offset, length), false));
		}
		...
		public bool IsChildOf(ExtPubKey parentKey)
		{
			if (Depth != parentKey.Depth + 1)
				return false;
			return new HDFingerprint(parentKey.PubKey) == ParentFingerprint;
		}
