...
using Pandora.Client.Crypto.Currencies.BouncyCastle.Crypto.Parameters;
using Pandora.Client.Crypto.Currencies.BouncyCastle.Security;
using Pandora.Client.Crypto.Currencies.Crypto.Internal;
using System;
...
namespace Pandora.Client.Crypto.Currencies.Crypto
{
    ...
    public static class SCrypt
    {
                                               ...
                                               int derivedKeyLength)
        {
            ...
            {
                lResult = kdf.Read(derivedKeyLength);
            }
		...
		{
			byte[] B = GetEffectivePbkdf2Salt(key, salt, cost, blockSize, parallel, maxThreads);
			var mac = new Pandora.Client.Crypto.Currencies.BouncyCastle.Crypto.Macs.HMac(new Pandora.Client.Crypto.Currencies.BouncyCastle.Crypto.Digests.Sha256Digest());
        ...
        public static byte[] CoinComputeDerivedKey(byte[] password, byte[] salt, int outputCount = 64)
        {
            return Pandora.Client.Crypto.Currencies.Crypto.SCrypt.ComputeDerivedKey(password, salt, 16384, 8, 8, 8, outputCount);
        }

        public static byte[] CoinComputeDerivedKey2(byte[] password, byte[] salt, int outputCount = 64)
        {
            return Pandora.Client.Crypto.Currencies.Crypto.SCrypt.ComputeDerivedKey(password, salt, 1024, 1, 1, 1, outputCount);
        }

        public static byte[] CoinComputeDerivedKey(string password, byte[] salt)
        {
            return CoinComputeDerivedKey(Encoding.UTF8.GetBytes(password), salt);
        }
    }
}