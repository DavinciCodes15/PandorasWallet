...
namespace Pandora.Client.Crypto.Currencies.RPC
{
	class BlockExplorerFormatter : RawFormatter
	{
        public BlockExplorerFormatter(Network aNetwork) : base(aNetwork)
        {
		...
		protected override void WriteTransaction(JsonTextWriter writer, Transaction tx)
		{
			...
			WritePropertyValue(writer, "size", tx.GetSerializedSize(tx.Version));
