...
namespace Pandora.Client.Crypto.Currencies
{
    ...
    public class Money : IComparable, IComparable<Money>, IEquatable<Money>, IMoney
    {
        ...
        public string ToString(bool fplus, bool trimExcessZero = true)
        {
            var fmt = string.Format("{{0:{0}{1}B}}",
                                    (fplus ? "+" : null),
                                    (trimExcessZero ? "2" : "8"));
            return string.Format(CoinFormatter.Formatter, fmt, _Satoshis);
        }
    ...
    internal class CoinFormatter : IFormatProvider, ICustomFormatter
    {
        public static readonly CoinFormatter Formatter = new CoinFormatter();
