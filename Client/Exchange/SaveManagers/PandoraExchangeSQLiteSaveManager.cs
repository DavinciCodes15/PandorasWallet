...
namespace Pandora.Client.Exchange.SaveManagers
{
    ...
    {
        private const int CURRENT_VERSION = 105;
        ...
        private void ApplyVersionChangesIfNeeded(string aUsername = null, string aEmail = null)
        {
            ...
            using (SQLiteConnection lConnection = new SQLiteConnection(FConnectionString))
            {
                ...
                if (lCurrentFileVersion < CURRENT_VERSION)
                {
                    ...
                    if (Version < 105)
                        Version105DoChangeUserOrderTable(lConnection);
        ...
        private void Version105DoChangeUserOrderTable(SQLiteConnection aOpenConnection)
        {
            using (var lSqliteTransaction = aOpenConnection.BeginTransaction())
                try
                {
                    ...
                    if (lColumns.Contains("BaseTicker"))
                        using (SQLiteCommand lCreateProfileTable = new SQLiteCommand("ALTER TABLE ExchangeTx RENAME COLUMN BaseTicker TO CurrencyTicker", aOpenConnection))
                            lCreateProfileTable.ExecuteNonQuery();
                    if (!lColumns.Contains("CurrencyID"))
                        using (SQLiteCommand lCreateProfileTable = new SQLiteCommand("ALTER TABLE ExchangeTx ADD COLUMN CurrencyID integer", aOpenConnection))
                            lCreateProfileTable.ExecuteNonQuery();
                    lSqliteTransaction.Commit();
                }
                catch (Exception ex)
        ...
        private void Version104DoAddOrderExchangeIDRelationshipTable(SQLiteConnection aOpenConnection)
        {
            using (var lSqliteTransaction = aOpenConnection.BeginTransaction())
                try
                {
                    ...
                    using (SQLiteCommand lInsertOldOrdersData = new SQLiteCommand($"INSERT INTO Relationship_ExchangeID_OrderID SELECT internalid, {(int) AvailableExchangesList.Bittrex} FROM ExchangeTx", aOpenConnection))
