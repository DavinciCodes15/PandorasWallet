...
namespace Pandora.Client.Exchange.CustomAPI.Poloniex.Net
{
    public class PoloniexClient : RestClient
    {
        ...
        /// </summary>
        public PoloniexClient(PoloniexClientOptions options) : base("Poloniex", options, options.ApiCredentials == null ? null : new PoloniexAuthenticationProvider(options.ApiCredentials))
        {
            requestBodyFormat = RequestBodyFormat.FormData;
        }
        ...
        public async Task<WebCallResult<IEnumerable<PoloniexChartData>>> GetChartDataAsync(string aCurrencyPair, long aUnixTimeStart, long aUnixTimeEnd, long aPeriod, CancellationToken act = default)
        {
            var lURI = $"https://poloniex.com/public?command=returnChartData&currencyPair={aCurrencyPair}&start={aUnixTimeStart}&end={aUnixTimeEnd}&period={aPeriod}";
            return await Execute<IEnumerable<PoloniexChartData>>(new Uri(lURI), HttpMethod.Get, act).ConfigureAwait(false);
        }

        public WebCallResult<IEnumerable<PoloniexChartData>> GetChartData(string aCurrencyPair, long aUnixTimeStart, long aUnixTimeEnd, long aPeriod, CancellationToken act = default)
        {
            return GetChartDataAsync(aCurrencyPair, aUnixTimeStart, aUnixTimeEnd, aPeriod, act).Result;
        }
        ...
        {
            return GetResult(await SendRequestAsync<T>(uri, method, ct, parameters, signed).ConfigureAwait(false));
        }
        ...
        protected override async Task<WebCallResult<T>> GetResponseAsync<T>(IRequest request, JsonSerializer deserializer, CancellationToken cancellationToken)
        {
            ...
            {
                log.Write(Microsoft.Extensions.Logging.LogLevel.Warning, "Request exception: " + requestException.Message);
            ...
            catch (TaskCanceledException canceledException)
            {
                if (canceledException.CancellationToken == cancellationToken)
                {
                    ...
                    log.Write(Microsoft.Extensions.Logging.LogLevel.Warning, "Request cancel requested");
                    return new WebCallResult<T>(null, null, default, new CancellationRequestedError());
                }
                else
                {
                    // Request timed out
                    log.Write(Microsoft.Extensions.Logging.LogLevel.Warning, "Request timed out");
